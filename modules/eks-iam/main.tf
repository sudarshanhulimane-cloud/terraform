...
resource "aws_iam_role" "eks_fargate_pod_execution" {
  ...
  tags = merge(var.tags, {
    Name = "${var.name}-eks-fargate-pod-execution-role"
    Type = "eks-fargate-pod-execution-role"
  })
}
...
resource "aws_iam_role" "aws_load_balancer_controller" {
  ...
  # Placeholder assume role policy - will be updated after OIDC provider is created
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
  ...
  tags = merge(var.tags, {
    Name = "${var.name}-aws-load-balancer-controller"
    Type = "aws-load-balancer-controller-role"
  })
}
...
resource "aws_iam_policy" "aws_load_balancer_controller" {
  ...
  tags = merge(var.tags, {
    Name = "${var.name}-AWSLoadBalancerControllerIAMPolicy"
    Type = "aws-load-balancer-controller-policy"
  })
}
...
# Note: OIDC Provider will be created separately after EKS cluster is created
# This module only creates the IAM roles needed for cluster creation